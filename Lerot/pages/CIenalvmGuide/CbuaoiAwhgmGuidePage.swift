...
struct CbuaoiAwhgmGuidePage: View {
    @StateObject private var cbuanxjInitMedel: EwuxbakAUkxBInitVModel = EwuxbakAUkxBInitVModel()
    @EnvironmentObject private var cbuaoNavi: NavigationManager
    @State private var cbuawjxIsShowLocation: Bool = false
    ...
    ZStack(alignment: .bottom) {
      ...
      VStack(spacing: 0) {
        ...
        Text("Welcome to LEROT")
          ...
          .padding(.bottom, 53)
          switch cbuanxjInitMedel.yuryxnAhwifkStatus {
          case .loading:
              VStack(spacing: 40) {
                ProgressView()
          ...
          case .xmaalwiDAiB:
              Button(action: {
                  Task {
                      cbuanxjInitMedel.yuryxnAhwifkStatus = .loading
                    if let route = await UwjalxiAecInitUtils.shared.yuryxnAhwifkGoLogin() {

                      await MainActor.run {
                          cbuaoNavi.popToRoot()
                          cbuaoNavi.push(route)
                      }
                    }
                      cbuanxjInitMedel.yuryxnAhwifkStatus = .xmaalwiDAiB
                  }
              }) {
                HStack(spacing: 40) {
        ...
        .padding(.bottom, 20)
        if cbuawjxIsShowLocation {
            BnexuwAkwLocationAlert(vhjreAxjkwuaIsShow: $cbuawjxIsShowLocation)
        }
        
    }.navigationBarHidden(true)
          .onChange(of: cbuanxjInitMedel.nextRoute) { route in
              if let route = route {
                  cbuaoNavi.push(route)
                  cbuanxjInitMedel.nextRoute = nil  // 防止重复跳转
              }
            }
          .environmentObject(cbuanxjInitMedel)
          .task{
              await cbuanxjInitMedel.lgirjAlakaBInit()
          }
  }
    
    struct CbuaoidAwhgGuideToA: View {
